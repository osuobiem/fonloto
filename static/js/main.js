;(function($) {
  ...
  $(document).ready(function() {
    ...
    // anchor tag disabled
    $('.lottery-result .result-board .table tbody tr td ul li a').on(
      'click',
      function(e) {
        e.preventDefault()
      }
    ...
    $('.tooltip-bottom').on('click', function() {
      ...
      if (item.length < 1) {
        $('.all-orders').append(
          "<h2>You don't have any tickets. Play now to add tickets to your order.</h2>"
        )
        $('.all-orders')
          .find('h2')
          .addClass('cart-empty-title')
    ...
    $('.jackpot-slider .owl-item.active')
      .eq(1)
      .addClass('target')
